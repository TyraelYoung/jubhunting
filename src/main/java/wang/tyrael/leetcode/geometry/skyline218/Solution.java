...
public class Solution {
    ...
    class MergeEventWindow{
        List<Event> endSortedEvents = new LinkedList<>();
        Map<Integer, Event> heightToEvent = new HashMap<>();

        void merge(Event newEvent){
            Event oldSameHeightEvent = heightToEvent.get(newEvent.height);
            if (oldSameHeightEvent == null) return;

        }

        Set<Event> removeEndBefore(int xScanLine){
            return null;
        }

        /**
         * newEventStart之前的event移除；如果有相同高度的则续命
         * @param newEvent
         * @return
         */
        Set<Event> move(Event newEvent){
            merge(newEvent);
            return removeEndBefore(newEvent.start);
        }
    }
